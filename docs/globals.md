[**@fest-lib/object API Documentation v0.1.4**](README.md)

***

# @fest-lib/object API Documentation v0.1.4

## Classes

- [DoubleWeakMap](classes/DoubleWeakMap.md)
- [AssignObjectHandler](classes/AssignObjectHandler.md)

## Interfaces

- [PropStore](interfaces/PropStore.md)
- [refWrap](interfaces/refWrap.md)

## Type Aliases

- [refType](type-aliases/refType.md)
- [TriggerName](type-aliases/TriggerName.md)
- [TriggerFilterList](type-aliases/TriggerFilterList.md)
- [AffectedOptions](type-aliases/AffectedOptions.md)
- [AffectedConfig](type-aliases/AffectedConfig.md)
- [AffectedCallback](type-aliases/AffectedCallback.md)
- [EffectEvent](type-aliases/EffectEvent.md)
- [EffectCallback](type-aliases/EffectCallback.md)
- [EffectOptions](type-aliases/EffectOptions.md)
- [EffectConfig](type-aliases/EffectConfig.md)
- [TriggerControl](type-aliases/TriggerControl.md)
- [observeValid](type-aliases/observeValid.md)
- [subValid](type-aliases/subValid.md)

## Variables

- [conditional](variables/conditional.md)
- [assignMap](variables/assignMap.md)
- [specializedSubscribe](variables/specializedSubscribe.md)
- [subscribeDirectly](variables/subscribeDirectly.md)
- [subscribeInput](variables/subscribeInput.md)
- [subscribePaired](variables/subscribePaired.md)
- [subscribeThenable](variables/subscribeThenable.md)
- [$triggerLess](variables/$triggerLess.md)
- [$triggerLock](variables/$triggerLock.md)
- [$triggerControl](variables/$triggerControl.md)
- [$trigger](variables/$trigger.md)
- [$affected](variables/$affected.md)

## Functions

- [conditionalIndex](functions/conditionalIndex.md)
- [conditionalRef](functions/conditionalRef.md)
- [remap](functions/remap.md)
- [unified](functions/unified.md)
- [observableBySet](functions/observableBySet.md)
- [observableByMap](functions/observableByMap.md)
- [assign](functions/assign.md)
- [link](functions/link.md)
- [computed](functions/computed.md)
- [delayedSubscribe](functions/delayedSubscribe.md)
- [useObservable](functions/useObservable.md)
- [affected](functions/affected.md)
- [effect](functions/effect.md)
- [effected](functions/effected.md)
- [makeArrayObservable](functions/makeArrayObservable.md)
- [iterated](functions/iterated.md)
- [unaffected](functions/unaffected.md)
- [bindBy](functions/bindBy.md)
- [derivate](functions/derivate.md)
- [bindByKey](functions/bindByKey.md)
- [numberRef](functions/numberRef.md)
- [stringRef](functions/stringRef.md)
- [booleanRef](functions/booleanRef.md)
- [wrapRef](functions/wrapRef.md)
- [propRef](functions/propRef.md)
- [$ref](functions/$ref.md)
- [ref](functions/ref.md)
- [promised](functions/promised.md)
- [triggerWithDelay](functions/triggerWithDelay.md)
- [delayedBehavior](functions/delayedBehavior.md)
- [delayedOrInstantBehavior](functions/delayedOrInstantBehavior.md)
- [observe](functions/observe.md)
- [isObservable](functions/isObservable.md)
- [recoverReactive](functions/recoverReactive.md)
- [makeObjectAssignable](functions/makeObjectAssignable.md)
- [safe](functions/safe.md)
- [unwrap](functions/unwrap.md)
- [deref](functions/deref.md)
- [addToCallChain](functions/addToCallChain.md)
- [wrapSetAsArray](functions/wrapSetAsArray.md)
